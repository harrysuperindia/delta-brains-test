...
const JWTLogin = ({ loginProp, ...others }: { loginProp?: number }) => {
    ...
    return (
        <Formik
            initialValues={{
                email: 'thaihuuluongtdu@gmail.com',
                password: '123123',
                submit: null
            }}
            validationSchema={Yup.object().shape({
                email: Yup.string().email('Email không đúng định dạng').max(255).required('Email không được trống'),
                password: Yup.string().max(255).required('Pasword không được trống')
            })}
